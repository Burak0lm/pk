...
prop_instance_receive_damage                      = 1877 # (prop_instance_receive_damage, <prop_instance_id>, <agent_id>, <damage>, [<advanced>]),
                                                         # <prop_instance_id> received <damage> damage from <agent_id>.
...
player_slot_gt                                    = 3010 # (player_slot_gt, <player_id>, <slot_no>, <value>), # Fails if <player_id>'s <slot_no> is not greater than <value>
team_slot_gt                                      = 3011 # (team_slot_gt, <team_no>, <slot_no>, <value>), # Fails if <team_no>'s <slot_no> is not greater than <value>
agent_slot_gt                                     = 3012 # (agent_slot_gt, <agent_id>, <slot_no>, <value>), # Fails if <agent_id>'s <slot_no> is not greater than <value>
scene_prop_slot_gt                                = 3013 # (scene_prop_slot_gt, <prop_instance_id>, <slot_no>, <value>), # Fails if <prop_instance_id>'s <slot_no> is not greater than <value>
...
send_message_to_url_advanced                      = 3019 # (send_message_to_url_advanced, <url_string>, <user_agent_string>, [<success_callback_script_no>], [<failure_callback_script_no>], [<skip_parsing>], [<timeout>]),
                                                         # Sends a HTTP request to <url_string> with <user_agent_string>. If the request succeeds, [<success_callback_script_no>] will be called.
                                                         # The script will behave like game_receive_url_response, unless [<skip_parsing>] is non-zero, in which case the script will receive no arguments and s0 will contain the full response.
...
dict_load_file                                    = 3202 # (dict_load_file, <dict>, <file>, [<mode>]), # Loads a dictionary file into <dict>.
                                                         # Setting [<mode>] to 0 (default) clears <dict> and then loads the file, setting [<mode>] to 1 doesn't clear <dict> but overrides any key that's already present, [<mode>] to 2 doesn't clear <dict> and doesn't overwrite keys that are already present
dict_load_dict                                    = 3203 # (dict_load_dict, <dict_1>, <dict_2>, [<mode>]), # Loads <dict_2> into <dict_1>. [<mode>]: see above
dict_save                                         = 3204 # (dict_save, <dict>, <file>),
...
dict_get_str                                      = 3210 # (dict_get_str, <string_register>, <dict>, <key>, [<default>]),
                                                         ...
                                                         # If the key is not found and [<default>] is set, [<default>] will be stored instead. If [<default>] is not set, an empty string will be stored
dict_get_int                                      = 3211 # (dict_get_int, <destination>, <dict>, <key>, [<default>]),
                                                         # Stores the numeric value paired to <key> into <destination>. If the key is not found and [<default>] is set, [<default>] will be stored instead.
...
agent_get_item_modifier                           = 3300 # (agent_get_item_modifier, <destination>, <agent_id>), # Stores <agent_id>'s horse item modifier (-1 if agent is not a horse) into <destination>
agent_get_item_slot_modifier                      = 3301 # (agent_get_item_slot_modifier, <destination>, <agent_id>, <item_slot_no>), # Stores <agent_id>'s <item_slot_no> modifier into <destination>
agent_get_animation_progress                      = 3302 # (agent_get_animation_progress, <destination>, <agent_id>, [<channel_no>]), # Stores <agent_id>'s channel [<channel_no>] animation progress (in %%) into <destination>
agent_get_dna                                     = 3303 # (agent_get_dna, <destination>, <agent_id>), # Stores <agent_id>'s dna into <destination>
agent_get_ground_scene_prop                       = 3304 # (agent_get_ground_scene_prop, <destination>, <agent_id>), # Stores the prop instance on which <agent_id> is standing into <destination>
agent_set_item_slot_ammo                          = 3305 # (agent_set_item_slot_ammo, <agent_id>, <item_slot_no>, <value>), # Sets <agent_id>'s <item_slot_no> ammo count to <value>
agent_get_item_slot_hit_points                    = 3306 # (agent_get_item_slot_hit_points, <destination>, <agent_id>, <item_slot_no>), # Stores <agent_id>'s <item_slot_no> shield hitpoints into <destination>
agent_set_item_slot_hit_points                    = 3307 # (agent_set_item_slot_hit_points, <agent_id>, <item_slot_no>, <value>), # Sets <agent_id>'s <item_slot_no> shield hitpoints to <value>
agent_get_wielded_item_slot_no                    = 3308 # (agent_get_wielded_item_slot_no, <destination>, <agent_id>, [<hand_no>]), # Stores <agent_id>'s wielded item slot for [<hand_no>] into <destination>
agent_get_scale                                   = 3309 # (agent_get_scale, <destination_fixed_point>, <agent_id>), # Stores <agent_id>'s scale into <destination_fixed_point>
agent_set_forced_lod                              = 3310 # (agent_set_forced_lod, <agent_id>, <lod_level>), # Forces <agent_id>'s LOD level to <lod_level> (0 = auto)
agent_get_item_slot_flags                         = 3311 # (agent_get_item_slot_flags, <destination>, <agent_id>, <item_slot_no>), # Stores <agent_id>'s <item_slot_no> item slot flags into <destination>
agent_ai_get_move_target_position                 = 3312 # (agent_ai_get_move_target_position, <position_register>, <agent_id>), # Stores <agent_id>'s move target position agent into <position_register>
agent_set_horse                                   = 3313 # (agent_set_horse, <agent_id>, <horse_agent_id>), # Sets <agent_id>'s horse to <horse_agent_id> (-1 for no horse)
agent_ai_set_simple_behavior                      = 3314 # (agent_ai_set_simple_behavior, <agent_id>, <simple_behavior>, [<guaranteed_time>]),
                                                         # Sets <agent_id>'s behavior to <simple_behavior> and guarantees it won't be changed for [<guaranteed_time>] seconds.
...
str_store_player_ip                               = 4230 # (str_store_player_ip, <string_register>, <player_id>), # Stores <player_id>'s IP address into <string_register> (works only on servers)
...
edit_mode_get_selected_prop_instance              = 4601 # (edit_mode_get_selected_prop_instance, <destination>, <index>), # Stores the <index>th selected prop instance into into <destination>
edit_mode_select_prop_instance                    = 4602 # (edit_mode_select_prop_instance, <prop_instance_id>), # Stores the <1>th selected prop instance into <prop_instance_id>
edit_mode_deselect_prop_instance                  = 4603 # (edit_mode_deselect_prop_instance, <prop_instance_id>), # Stores the <1>th selected prop instance into <prop_instance_id>
...
edit_mode_set_highlighted_prop_instance           = 4605 # (edit_mode_set_highlighted_prop_instance, [<prop_instance_id>]), # Stores the <1>th selected prop instance into [<prop_instance_id>]
...
menu_create_new                                   = 4800 # (menu_create_new, <destination>, <text>, [<mesh_name>], [<flags>], [<script_no>], [<script_param>]),
                                                         # Creates a dynamic menu and stores its id into <destination>. [<script_no>] (-1 for no script) will be called with params:
                                                         # 1 = menu_no, 2 = [<script_param>] when the operations block is executed
menu_add_item                                     = 4801 # (menu_add_item, <menu_no>, <text>, [<conditions_script_no>], [<consequences_script_no>], [<script_param>]),
                                                         # Adds a new menu item to <menu_no>. [<conditions_script_no>] and [<consequences_script_no>] (-1 for no script) will be called with params:
                                                         # 1 = <menu_no>, 2 = [<script_param>] when the conditions/consequences blocks are executed
