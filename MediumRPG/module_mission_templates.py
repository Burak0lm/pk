...
after_mission_start_setup = (ti_after_mission_start, 0, 0, [], # spawn and move certain things after most other set up is done
   [(set_spawn_effector_scene_prop_kind, 0, -1),
    ...
    (assign , "$g_script_licence" , 0),
    (try_for_prop_instances , ":instance_id" , "spr_pw_item_chest_a"),
          (prop_instance_get_position , pos1 , ":instance_id"),
          (lua_push_int , ":instance_id"),
          (lua_call , "@loadChest" , 1),
    (try_end),
    (try_for_prop_instances , ":instance_id" , "spr_pw_item_chest_b"),
          (prop_instance_get_position , pos1 , ":instance_id"),
          (lua_push_int , ":instance_id"),
          (lua_call , "@loadChest" , 1),
    (try_end),
    (lua_call , "@helloWorld" , 0),
    (call_script , "script_karma_licence_check"),
    ])
...
auto_save_loop = (7200, 0, 0,[], [
   (multiplayer_is_server),
      
      (call_script , "script_karma_licence_check"),
      (try_for_players , ":player_id"),
        (call_script , "script_auto_save_players" , ":player_id"),
      (try_end),
   ])
agent_starving_loop = (0, 0, 360,[
  (try_for_players , ":player_id"),
      ...
      #(call_script , "script_cf_check_starving" , ":player_id" , ":agent_id"), acligi kaldirdik
  (try_end),
  ] ,[])
...
agent_spawn = (ti_on_agent_spawn, 0, 0, [], # server and clients: set up new agents after they spawn
   [(store_trigger_param_1, ":agent_id"),
    (try_begin),
      (gt , "$g_script_licence" , 0),
      (call_script, "script_on_agent_spawned", ":agent_id"),
     (else_try),
       (display_message , "@### SCRIPT ZYTHEIN TARAFINDAN DEAKTIF EDLIMISTIR (STEAM = ZYTHEIN) ###"),
   ...
   [(store_trigger_param_1, ":dead_agent_id"),
    ...
    (call_script, "script_check_spawn_bots", ":dead_agent_id"),
     (try_begin),
      ...
      #(call_script , "script_check_killed_by" , ":dead_agent_id" , ":killer_agent_id"),
    (try_end),
...
player_check_loop = (0, 0, 0.5, # server: check all players to see if any need agents spawned, also periodically lowering outlaw ratings
   [(multiplayer_is_server),
    ...
    (try_for_range, ":player_id", "$g_loop_player_id", ":loop_end"), # continue from the last player id checked
      ...
      (player_get_slot , ":wound_time" , ":player_id" , slot_player_wounded_time),
      (try_begin),
        (neq , ":wound_time" , 0),
        (ge , ":time" , ":wound_time"),
        (player_set_slot , ":player_id" , slot_player_is_wounded , 0),
        (multiplayer_send_string_to_player , ":player_id" , server_event_local_chat_shout , "@Bilinciniz acilmaya basladi. ( Yarali rolune devam ediniz )"),
        (player_set_slot , ":player_id" , slot_player_wounded_time , 0),
        (player_set_slot , ":player_id" , slot_player_next_spawn_health_percent , 10),
        (player_get_slot , ":meslek" , ":player_id" , slot_player_wound_class),
        (player_set_troop_id , ":player_id" , ":meslek"),
